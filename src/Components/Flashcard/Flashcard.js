  ...
  return (
    <SwitchTransition >
      <CSSTransition key={show+'version'} classNames="bouncer" timeout={1000}>
        ...
        { !show && <img src={path} className="row-start-1 row-end-4 w-full h-full object-contain " alt="zdjęcie związane z pytaniem" />}

          <Box marginY={8} className="row-start-4">
            {" "}
            <h2 className="bold-serif">{show ? "Odpowiedź" : "Pytanie"}</h2>
            <p className="block ">{show ? answer : question}</p>
          </Box>
        </div>
      </CSSTransition>
    </SwitchTransition>
