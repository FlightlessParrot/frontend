...
export default function ImageForm({fileRef, check, setCheck, id_suffix}) {
    ...
    const input =useRef(Math.random().toString()+id_suffix)
  return (
    <FormControl  isInvalid={imageIsInvalid} >
    <FormLabel className='flex items-end p-4 cursor-pointer action-button gap-10' htmlFor={input.current}><Checkbox p={1} isChecked={check} readOnly/> Dodaj zdjęcie</FormLabel>
    <Text color={"gray.500"} size='xs'>Dopuszczalne formaty to jpg, png oraz webp</Text>
    <input  name='image' data-testid='fileInput' className="hidden" id={input.current} onChange={e=>{setDirty(true); fileRef.current.files[0]!=null ? setCheck(true): setCheck(false)}} type="file" accept=".jpg,.png,.webp" ref={fileRef}></input>
